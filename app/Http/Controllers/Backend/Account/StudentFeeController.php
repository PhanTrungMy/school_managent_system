...
class StudentFeeController extends Controller
{
    ...
    {
        $data['allData'] = AccountStudent::all();
    ...
    public function StudentFeeStore(Request $request){
      ...
      AccountStudent::where('year_id',$request->year_id)->where('student_id',$request->student_id)->where('fee_category_id',$request->fee_category_id)->where('date',$request->date)->delete();
      $checkedata = $request->checkmanage;
      if($checkedata != null){
        for ($i = 0; $i < count($checkedata); $i++){
            $data = new AccountStudent();
            $data->year_id = $request->year_id;
            $data->class_id = $request->class_id;
            $data->date = $date;
            $data->fee_category_id = $request->fee_category_id;
            $data->student_id = $request->student_id[$checkedata[$i]];
    ...
    public function StudentFeeGetStudent(Request $request)
    {
        ...
        foreach ($data as $key => $std) {
            ...
            $accountstudentfees = AccountStudent::where('student_id', $std->student_id)->where('year_id', $std->year_id)->where('class_id', $std->class_id)->where('fee_category_id', $fee_category_id)->where('date', $date)->first();
