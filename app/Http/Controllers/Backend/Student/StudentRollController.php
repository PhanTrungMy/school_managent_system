...
class StudentRollController extends Controller
{
 ...
 public function StudentGenerateRollView(Request $request){
  $allData = AssignStudent::with(['Student'])->where('year_id',$request->year_id)->where('class_id',$request->class_id)->get();
// dd($allData->toArray());
     return response()->json($allData);
 }
 public function StudentGenerateRollStore(Request $request){
     ...
     if ($request->student_id !==null){
         ...
         $notification =  array(
             'message' =>' Sorry There are no Student Successfully',
             'alert-type' => 'error'
         );
     ...
     $notification =  array(
         'message' =>'Well Roll Generate  Inserted Successfully',
         'alert-type' => 'success'
     );
